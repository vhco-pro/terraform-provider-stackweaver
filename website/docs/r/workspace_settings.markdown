---
layout: "stackweaver"
page_title: "Stackweaver: stackweaver_workspace_setting"
description: |-
...
~> **NOTE:** Manages or reads execution mode and agent pool settings for a workspace. This also interacts with the organization's default values for several settings, which can be managed with [stackweaver_organization_default_settings](organization_default_settings.html). If other resources need to identify whether a setting is a default or an explicit value set for the workspace, you can refer to the read-only `overwrites` argument.

~> **NOTE:** This resource manages values that can alternatively be managed by the  `stackweaver_workspace` resource. You should not attempt to manage the same property on both resources which could cause a permanent drift. Example properties available on both resources: `description`, `tags`, `auto_apply`, etc.
...
resource "stackweaver_workspace_settings" "test-settings" {
...
resource "stackweaver_agent_pool_allowed_workspaces" "test" {
  agent_pool_id         = stackweaver_agent_pool.test-agent-pool.id
  allowed_workspace_ids = [stackweaver_workspace.test.id]
}
...
resource "stackweaver_workspace_settings" "test-settings" {
  workspace_id   = stackweaver_workspace.test.id
  agent_pool_id  = stackweaver_agent_pool_allowed_workspaces.test.agent_pool_id
...
resource "stackweaver_workspace_settings" "test-settings" {
  for_each                  = toset(["qa","production"])
  workspace_id              = stackweaver_workspace.test[each.value].id
  global_remote_state       = false
  project_remote_state      = false
  remote_state_consumer_ids = toset(compact([each.value == "production" ? stackweaver_workspace.test["qa"].id : ""]))
}
```
...
resource "stackweaver_workspace_settings" "test" {
  workspace_id   = data.stackweaver_workspace.test.id
}

output "workspace-explicit-local-execution" {
  value = alltrue([
    stackweaver_workspace_settings.test.execution_mode == "local",
    stackweaver_workspace_settings.test.overwrites[0]["execution_mode"]
  ])
}
```
...
data "stackweaver_workspace" "self" {
  name         = "self-managed"
  organization = "foo"
}

resource "stackweaver_workspace_settings" "self" {
  workspace_id        = data.stackweaver_workspace.self.id
...
```shell
terraform import stackweaver_workspace_settings.test ws-CH5in3chf8RJjrVd
```

```shell
terraform import stackweaver_workspace_settings.test my-org-name/my-wkspace-name
```
