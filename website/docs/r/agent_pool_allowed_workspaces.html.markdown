---
layout: "stackweaver"
page_title: "Stackweaver: stackweaver_agent_pool_allowed_workspaces"
description: |-
...
In this example, the agent pool and workspace are connected through other resources that manage the agent pool permissions as well as the workspace execution mode. Notice that the `stackweaver_workspace_settings` uses the agent pool reference found in `stackweaver_agent_pool_allowed_workspaces` in order to create the permission to use the agent pool before assigning it.
...
resource "stackweaver_agent_pool_allowed_workspaces" "allowed" {
  agent_pool_id         = stackweaver_agent_pool.test-agent-pool.id
  allowed_workspace_ids = [stackweaver_workspace.test-workspace.id]
}

// Lastly, ensure the workspace agent execution is assigned last by
// referencing allowed_workspaces
resource "stackweaver_workspace_settings" "test-workspace-settings" {
  workspace_id   = stackweaver_workspace.test-workspace.id
  execution_mode = "agent"
  agent_pool_id  = stackweaver_agent_pool_allowed_workspaces.allowed.id
}
```
...
```shell
terraform import stackweaver_agent_pool_allowed_workspaces.foobar apool-rW0KoLSlnuNb5adB
```
